...
pacman::p_load("dplyr",          # Data wrangling
               ...
               "txBenefit",      # Cb
               "grf",            # Causal forests (Qini curve example)
...
ggplot(dat_arr,
  ...
  scale_y_continuous(name = "Absolute risk increase",
...
## S-learners{.smaller}
An example using data from one of the first succesful trials of adjuvant chemotherapy in colon cancer (Levamisole + 5-FU vs. placebo) ^[Moertel *et al.* 1995 Ann Intern Med; doi: [10.7326/0003-4819-122-5-199503010-00001](https://doi.org/10.7326/0003-4819-122-5-199503010-00001)]
...
ggplot(tibble(vec_ard),
    ...
    geom_histogram(binwidth = 0.01,
                   ...
                   fill = "#cde498") +
    # Scalings
    scale_x_continuous(name = "Absolute risk increase",
                       limits = c(-0.8, 0.2),
...
p_match <- 
    ...
    scale_x_continuous(breaks = seq(-0.6, 0.6, 0.3),
                       ...
                       name = "Predicted pairwise ARI") +
    scale_y_continuous(breaks = seq(-0.6, 0.6, 0.3),
                       labels = paste0(format(seq(-0.6, 0.6, 0.3) * 100, nsmall = 1), "%"),
                       name = "Observed pairwise ARI") +
    # Transformations
...
:::{.incremental}
- Efficiency of a prediction model in assigning treatment, compared to random treatment
- Uses random sampling of benefit
- Strong assumption: benefit is correctly calibrated; <br> 
...
:::{.incremental}
- Weight your outcome ($Y$) by the propensity score ($PS$): <br>
$Y^* = \frac{Y(A - PS)}{PS(1 - PS)}$ <br>
...
- Create a decision rule $\pi$ to treat the top $\phi$ percent: <br>
$\pi_\phi = \begin{cases}1, & if \hspace{0.1cm} ARD \ge top \hspace{0.11cm} \phi\%\\0, & otherwise\end{cases}$
- Calculate the gain corresponding to the proportion treated $g(\phi)$: <br>
...
:::{.incremental}
- Adjust individual benefit $ARD_i$ with individual cost $C_i$: <br>
$ARD^* = \frac{ARD_i}{C_i}$
...
- Calculate the cost corresponding to the proportion treated $c(\phi)$: <br>
$c(\phi) = \frac{1}{N_\phi}\displaystyle\sum^{N_\phi}_iC_i$
- Adjust treated proportion to account for incurred costs: <br>
$\phi^* = g(\phi) * c(\phi)$
...
# Example from https://grf-labs.github.io/grf/articles/maq.html 
n <- 2000
p <- 5
...
# Generate plot
qini(tau.hat, W[test], Y[test], 0.5)

```

::: aside
Simulated data from [Qini curves: Automatic cost-benefit analysis](https://grf-labs.github.io/grf/articles/maq.html)
:::
...
# Thank you!{background-image=images/polar_bear.jpeg background-opacity="0.3"}
...
- Code: [github.com/rjjanse](https://github.com/rjjanse/rjjanse.github.io/blob/main/talks/hte.qmd)